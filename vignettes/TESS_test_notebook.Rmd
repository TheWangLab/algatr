...
raster <- aggregate(CA_env[[1]], 4)

# alternatively a raster can be created from coordinates
raster2 <- coords_to_raster(liz_coords, res = 0.01, buffer = 1, plot = TRUE)
```


```{r, cache = TRUE, results = FALSE, warning = FALSE, fig.width = 5, fig.height = 5}
par(pty = "s", mar = rep(2,4))
results <- tess_full(gen, liz_coords, raster, Kvals = 1:3, K_selection = "auto", minQ = 0.10)
```
...
# krige Qmatrix
krig_admix <- tess_krig(qmat, liz_coords, raster)
```
