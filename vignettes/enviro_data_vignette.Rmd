...
```{r collin env, results='hide', fig.width = 10, fig.height = 10}
# Convert wclim from SpatRaster to RasterStack
wclim_stack <- stack(wclim)

# Check for collinearity; the result is a list containting (1) a dataframe of correlated environmental pairs that fall above the threshold and (2) a correlation matrix with all pairwise correlation coefficients
...
check_result <- check_vals(wclim_stack, liz_coords)
head(check_result$cor_pairs)
```
...
TODO[EAC]: THe instructions above need to be corrected. Currently the environmental distances can only be calculated with a euclidean distance. The geographic distance is the one that is flexible. Not sure how to explain that best.

```{r collin dists}
check_results <- check_dists(wclim_stack, liz_coords)
