...
-   `raster_pca()` to perform a raster PCA on a stack of environmental layers to reduce dimensionality
...
Because of collinearity issues, and because we may not want to deal with dozens of environmental layers at a time, we can perform a PCA on our rasters using the `raster_pca()` function. Let's perform a raster PCA on the `wclim` object we generated above using the `get_worldclim()` function, which contains 19 environmental variables. In many cases, the top three PCs may explain the majority of the variance of the data, and so only those will be considered for further analyses.
