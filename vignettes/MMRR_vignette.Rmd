...
Once again, the results from `mmrr_best()` contains the same elements as `mmrr_full()`, except that `mmrr_best()` now also contains an element *TODO[EAC]:missing a word? what element*. You can also see that the `coeff_df` element is reduced from all four variables to only those variables that were significantly associated with genetic distance (in the test example case, only geographic distance). There is also an additional element, `X_best`, which contains values for significant variables; in this case, the only significant variable found was geographic distance.
