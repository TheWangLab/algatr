...
There are a few assumptions built into this function that users should be aware of: (1) the coordinates and genetic distance files MUST have the same ordering of individuals; there isn't a check for this, and (2) this function assumes that each individual has its own sampling coordinates (i.e., multiple individuals weren't collected per site).
...
```{r mmrr full, fig.width = 5, fig.height = 5}
set.seed(10)
results_full <- mmrr_full(Y, X, nperm = 99, stdz = TRUE, plot = FALSE)
```
...
set.seed(01)
results_best <- mmrr_best(Y, X, nperm = 99, stdz = TRUE, plot = FALSE)
```

Once again, the results from `mmrr_best()` contains the same elements as `mmrr_full()`, except that `mmrr_best()` now also contains an element. You can also see that the `coeff_df` element is reduced from all four variables to only those variables that were significantly associated with genetic distance (in the test example case, only geographic distance). There is also an additional element, `X_best`, which contains TODO [EAC]: FILL IN values for significant variables; in this case, only geographic distance.
...
```{r mmrr plots, fig.width = 5, fig.height = 5}
...
```{r mmrr plots best, fig.width = 5, fig.height = 5}
mmrr_plot(Y, X, mod = results_best$mod, plot_type = "all", stdz = TRUE)
```

### Look at MMRR output statistics with `mmrr_table()`

We can also take a closer look at the output statistics from our MMRR runs using the `mmrr_table()` function. This function will also provide summary statistics if we set `summary_stats` to TRUE (the default).
...
set.seed(01)
mmrr_do_everything(liz_gendist, liz_coords, envlayers = CA_env, model = "full")
```
...
set.seed(10)
mmrr_do_everything(gendist, liz_coords, envlayers = CA_env, model = "best", nperm = 100)
```
