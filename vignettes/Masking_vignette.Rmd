...
1.  The `extrap_mask()` function extrapolates environmental values from enviro layers that will be masked.
...
plot_extrap_mask(envlayers, map_mask, RGB_cols = TRUE)
...
We can also mask all areas outside of a convex hull around the coordinates provided using the `"chull"` argument. As with the buffer-based masking, this masking is also agnostic to environment, and the size of the buffer is once again changed with the `buffer_width` option.
