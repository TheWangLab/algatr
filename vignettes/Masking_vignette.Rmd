...
-   `extrap_mask()` extrapolates environmental values from environmental layers that will be masked
...
First, let's get the shape (polygon) for the state of California. This will include the Channel Islands. TODO: We can retrieve administrative boundaries using the `gadm()` function within the geodata package.

```{r cali polygon}
# TODO [EAC]: shouldn't use getData for this - need to switch to geodata
