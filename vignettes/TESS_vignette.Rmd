...
We can create interpolated maps of the ancestry coefficient estimates using a method known as kriging which uses a spatially explicit model for spatial interpolation. To do so, we need a raster onto which we can map Q values. 

*TODO[EAC]: show how you can use the coords_to_raster function to do this. Also, while TESS itself provides an interpolation method I do not know if it is kriging. The code that I wrote uses kriging for interpolation but maybe check if TESS does the same. I don't think it does because the smoothed maps produced by TESS are different from the ones produced by my code. We may want to include a note about this (just briefly say TESS also does this but you can't get a raster output currently from it). You actually discuss this on line 224 a bit so maybe just add that up here*
...
algatr allows users with the option to test a range of K values using the `tess_ktest()` function. K selection is often accomplished by minimizing the cross entropy, but this can over-simplify selection. The `tess_ktest()` function allows users to specify how K selection is done using the `K_selection` argument; users can set the K values manually (`"manual"`) or using an automatic approach described below (`"auto"`). `tess_ktest()` runs TESS for each value of K within the user-specified range, and outputs cross-validation scores for different K values which allows a user to select the "best" K for their dataset.

### Manual K selection

Let's run TESS on a set of K values ranging from 1-10, and select the "best" K value manually based on cross-validation scores. To do so, we'll specify `"manual"` within the `K_selection` argument. Typically, researchers will select the "best" K value by minimizing cross-validation scores.

*TODO[EAC]: you can't run a manual selection step as part of a vignette because it won't knit/build properly*

```{r manual K selection, message = FALSE}
# In this case, we can see that the "best" K (minimizing xval score) is also the max value, 10
tess_ktest(liz_dosage, liz_coords, Kvals = 1:10, ploidy = 2, K_selection = "manual")
```
...
```{r auto K selection, message = FALSE}
...
1.  `K`: The value of the best-supported K (3, in this case); this is assigned a value of NA if manual selection is done

2.  `tess3_obj`: Results from the cross-validation analysis of all values of K

3.  `coords`: Sampling coordinates

4.  `Kvals`: The range of K values that were tested

5.  `grid`: The RasterLayer upon which ancestry coefficients are mapped; NULL in this case

### Running TESS with no K selection

If you want to run TESS without any K selection, you could just run TESS as normal, doing the following:
...
A typical representation for population structure results is a STRUCTURE-style barplot, in which each stacked bar represents an individual and the proportion of stacked color represents the proportion of ancestry assigned to that individual for each cluster (or K value). In our case, we have K=3, so there will be three colors representing each of these K values.

algatr has a custom function, `tess_barplot()`, to generate these plots from our TESS results. This automatically sort individuals based on their Q values (although this can be modified using the `sort_by_Q` argument, if so desired).
...
Now let's explore how Q values (ancestry coefficients) are mapped. The `tess_ggplot()` function will take in the kriged admixture values and sampling coordinates, and provides several options for the plot method (with the `plot_method` argument):
...
```{r basic ggplot, fig.width = 5, fig.height = 5}
...
In many cases, a user may want more customizability in mapping their TESS results. For example, users may want to have more control over color breaks, alpha levels, legend/color scale settings. In this case, we can use base R plotting functions with the `tess_plot()` function.

```{r extended mapping, fig.width = 5, fig.height = 5, warning = FALSE}
...
```{r plot all K, fig.width = 10, fig.height = 4}
...
```{r tess default barplot}
barplot(qmat, sort.by.Q = TRUE, border = NA, space = 0, xlab = "Individuals", ylab = "Ancestry coefficients")
```

You can also map kriged Q-values using tess3r with the `plot()` function. *NOTE: Be aware that the original tess3r package uses a different type of kriging than autoKrige. tess3r also kriges and plots in the same function, so there isn't an easy way to get the raster output.*
...
The algatr package also has an option to run all of the above functionality in a single function, `tess_do_everything()`. *(N.B.: the `tess_do_everything()` function does not require a dosage matrix; it will do the conversion automatically if a vcf is provided)*.

The `tess_do_everything()` function will print out the barplot, the cross-validation scores for the range of K values tested, and the kriged map. It will also print the order in which individuals are ordered in the barplot. The resulting object from this function contains:

-   `K`: The best K value based on user-specified K selection

-   `Qmatrix`: The matrix of individual ancestry coefficients, or Q values

-   `krig_admix`: A RasterBrick object containing the kriged ancestry coefficient values for mapping
...
# TODO [EAC]: fix par so they're not stacked
# TODO [EAC]: best K is now 2 vs 3 as above?
results <- tess_do_everything(liz_vcf, liz_coords, CA_env[[1]], Kvals = 1:3, K_selection = "auto", minQ = 0.10)
```
