...
The `tess_krig()` function will take in ancestry coefficient values (in the Q-matrix) and will krige the values based on the raster provided (`krig_raster` from above). This will produce a Raster\* type object. There are two kriging options incorporated into this function, specified using the `krig_method` argument; the first (and default) is `"autoKrige"` which uses the automatic kriging `autoKrige()` function within the `automap` package and the second is `"fields"` which uses the `Krig()` function within the `fields` package. The latter is used by the tess3r package.
...
krig_admix <- tess_krig(qmat, coords_proj, krig_raster, krig_method = "fields")
```
