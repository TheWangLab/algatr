...
In many cases, a user may want more customizability in mapping their TESS results. For example, users may want to have more control over color and whether to display axes, both of which can be done using the `ggplot_fill` and `plot_axes` arguments, respectively:
...
results <- tess_do_everything(liz_vcf, liz_coords, grid = CA_env[[1]], Kvals = 1:5, K_selection = "auto")
```
