...
TESS provides the ability to map ancestry coefficient estimates, and smooths the resulting surface using a method of spatial interpolation called kriging. We need to download a raster onto which we can map Q values.
...
The `tess_krig()` function will take in ancestry coefficient values (in the Q-matrix) and will krige the values based on the raster provided (`krig_raster` from above). This will produce a RasterBrick object.
