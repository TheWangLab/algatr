...
To understand what the above `tess_do_everything()` function is doing internally, we can run TESS3 line-by-line. First, let's test out different K values using the `tess_ktest()` function. The resulting object, `XX`, tells us the cross-validation scores for the different K values that were tested.

```{r test Ks}
tess3_result <- tess_ktest(gen, liz_coords, Kvals = 1:3, ploidy = 2)
