...
To generate a map, we need a raster onto which we can map Q values. We can either retrieve this, or generate one ourselves using the `coords_to_raster()` function. Please refer to the environmental data vignette for further information on this function.
...
A user can run TESS on a set of K values ranging from 1-10, and select the "best" K value manually based on cross-validation scores. To do so, a user must specify `"manual"` within the `K_selection` argument. Typically, researchers will select the "best" K value by minimizing cross-validation scores.
...
-   `K`: The value of the best-supported K (3, in this case); this is assigned a value of `NA` if manual selection is done
...
```{r plot all K, fig.width = 10, fig.height = 4, fig.align = 'center'}
...
results <- tess_do_everything(liz_vcf, liz_coords, grid = CA_env[[1]], Kvals = 1:10, K_selection = "auto", minQ = 0.10)
```
