...
Running TESS3 requires three data files: a genotype dosage matrix (the `gen` argument), coordinates for samples (the `coords` argument), and environmental layers (the `envlayers` argument). We can use a vcf and the `vcf_to_dosage()` function in the wingen package to convert a vcf to a dosage matrix.
...
```{r dosage}
# Convert vcf to genotype matrix using wingen
