...
-   **Variance partitioning:** Considering only those variables that contribute significantly to genetic variance from variable selection ("best" model), we can then perform variance partitioning to determine the relative contributions of each variable to genetic variance (done by running pRDAs compared to a simple RDA with the "best" variables)

-   **Detecting outlier loci:** Based on the results from above, select a model based on significant variables and detect outlier loci with `rda_getoutliers()`
...
Now, we see that even more inertia/variance is explained by the conditioned variables.
...
TODO EAC: mention MAF filtering / LD pruning (diff from Capblancq and Forester)
...
TODO EAC: if I add varpart to rda code, add it below

The algatr package also has an option to run all of the above functionality in a single function, `rda_do_everything()`. This function is similar in structure to `rda_run()`, and also generate figures and the correlation test table by default. Let's first run a simple RDA with variable selection and the p-value outlier method:

```{r simple RDA do everything, warning=FALSE}
...
Now, let's run a partial RDA, correcting for both geography and structure, and instead using the Z-score method to detect outlier loci:

```{r pRDA do everything, warning=FALSE}
