...
One of the most relevant statistics to present from an RDA model is the adjusted R2 value. The R2 value *must* be adjusted because otherwise it is biased **(TODO[EAC]: prob should explain why/how it is biased/just one sentence about # variables)**. The adjusted R2 value from the full (global) model can also help us determine the stopping point when we go on to do variable selection. As we can see, the unadjusted R2 is 0.098, while the adjusted value is 0.042. Be sure to always report *adjusted* R2 values.
...
```{r intersect}
Reduce(intersect, list(q_sig$snp_names,
                       rda_sig_p$rda_snps,
                       rda_sig_z$rda_snps))
```
