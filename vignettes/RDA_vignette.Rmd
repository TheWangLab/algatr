...
Running an RDA requires three data files for input: a genotype dosage matrix (the `gen` argument), coordinates for samples (the `coords` argument), and environmental values extracted for sampling coordinates (the `env` argument). If you do not have extracted environmental values (as with the test dataset), `rda_do_everything()` will extract values for us.
...
A partial RDA describes incorporating covariables into a redundancy analysis. For more information, there is an excellent description of the difference between a simple and partial RDA in [Capblancq and Forester 2021](https://besjournals.onlinelibrary.wiley.com/doi/10.1111/2041-210X.13722).
