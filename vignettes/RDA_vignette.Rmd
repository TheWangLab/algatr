...
Let's extract environmental variables using the `extract()` function from the raster package. TODO EXPLAIN STDZ
...
A simple RDA is one in which our model will not account for covariables in the model.
...
mod <- rda_run(gen, env, 
               ...
               correctGEO = FALSE,
               correctPC = FALSE,
               Pin = 0.05, # only if model=best limits of permutation P-values for adding (`Pin`) a term to the model, or dropping (`Pout`) from the model. Term is added if` P <= Pin`, and removed if `P > Pout` (see \link[vegan]{ordi2step})
               R2permutations = 1000, # only if model=best number of permutations used in the estimation of adjusted R2 for cca using RsquareAdj (see \link[vegan]{ordi2step})
               R2scope = T) # only if model=best use adjusted R2 as the stopping criterion: only models with lower adjusted R2 than scope are accepted (see \link[vegan]{ordi2step})
```
