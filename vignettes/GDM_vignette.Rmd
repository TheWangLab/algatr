...
TODO[EAC]: modify this so it is clear that this works for population based data (i.e., you should be able to provide allele frequencies/site based genetic distances and site locations instead of dosages/individual based genetic distances and individual locations). For example, if population based sampling is performed you can either use the individuals or the populations its just that the genetic data has to match.
...
TODO[EAC]: we should make it so that for gdm_run env can be a dataframe or a raster stack (just add a check at the start of the function that if env is a raster object (use the inherits function and check for RasterLayer and RasterStack or whatever they're called) you run raster::extract and if not you just leave it as is).

```{r gdm full}
gdm_full <- gdm_run(gendist = liz_gendist, coords = liz_coords, env = env, model = "full", scale_gendist = TRUE)
```
...
*TODO [EAC]: REMOVE COMMENTS ON BELOW LINES*.
...
Notice that the `pvalues` and `varimp` elements are no longer NULL (as they were with the full model); this is because model selection was performed. `pvalues` indicate the significance of each environmental variable with genetic distance (given the user-defined threshold), and `varimp` contains *TODO EAC DESCRIBE VARIMP; REMOVE COMMENTS ON BELOW LINES*.
...
```{gdm full summary, warning = FALSE}
...
```{r isplines, fig.width = 3, fig.height = 3}
par(mfrow = c(2,2))
...
```{r gdm masked, warning=FALSE, fig.align='center', fig.width=5, fig.height=5}
...
The algatr package also has an option to run all of the above functionality in a single function, `gdm_do_everything()`. This function will output the fitted I-splines, table, map, and PCA. The resulting object looks identical to the output object from `gdm_run()`.
...
```{r do everything, fig.width=5, fig.height=5, fig.align='center', results='hide'}
gdm_full_everything <- gdm_do_everything(liz_gendist, liz_coords, envlayers = CA_env, model = "full", scale_gendist = TRUE)
```
