...
To run GDM, we need to generate a site-pair table in which variables are extracted for each site. Because we only have environmental layers for our example dataset, we need to extract environmental variables from these layers for each sampling locality in which lizards were collected:
...
### Comparing between full and best GDM models

*TODO [EAC]: need to fill this in once I get gdm best running*

### Table of GDM statistics

We can generate a nice table with relevant GDM statistics to report using the `gdm_table()` function.
