...
For this vignette, we'll be using a subset of the *Sceloporus occidentalis* RADseq dataset from [Bouzid et al. 2022](REFER) which contains 1,000 SNPs for 53 individuals.
...
gdm_best <- gdm_run(gendist = gendist, coords = liz_coords, env = env, model = "best", scale_gendist = TRUE, nperm = 50, sig = 0.05)
...
We can generate a nice table with relevant GDM statistics to report using the `gdm_df()` function to extract from our GDM results object, and then build the table itself with the `gdm_table()` function. The coefficients contained in this table are the sum of the three I-spline coefficients we saw above when we looked at the `coefficients` element within the GDM model.
...
Secondly, to understand the relative contributions explained by IBE (each of the environmental predictor variables) and IBD (geographic distance), we sum the three I-spline coefficients for each variable; non-zero sums are variables that are significantly associated with genetic dissimilarity. We can visualize how genetic composition is associated with each of these variables by plotting each as a vector.
