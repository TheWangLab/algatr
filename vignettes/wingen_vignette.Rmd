...
**If using wingen, please cite the following: Bishop A.P., Chambers E.A., Wang I.J. In review.**

wingen is a package that uses a moving window approach to create continuous maps of genetic diversity. Please see [Bishop et al. 2022](TODO REFER) for the paper describing the method, and refer to the [wingen package](https://github.com/AnushaPB/wingen) for further documentation.
...
**TODO[EAC]: prevew_gd doesn't visualize window_gd, it provides a reference point for what the window and focal cell looks like relative to the landscape and what the sample count in each cell is given the parameters (idk how you want to incorporate this into the vignette)**
...
```{r coords to raster}
liz_lyr <- coords_to_raster(liz_coords, res = 0.5, buff = 1)
```
...
**TODO[EAC]:`fact` isn't just for decreasing computational speed, it can also help make the raster more continuous since each window will contain more samples. I have removed this point in some place, but you can add both back in (just don't only say the computational speed thing)**
...
Finally, we may want to mask out any areas that we're not interested in using the `mask_gd()` function. wingen provides several ways to perform masking, but we will focus on two for this vignette. The `mask_gd()` function is quite simple in that it takes in the layer you want masked (`kgd`, in our case) and the layer you want to mask to. Let's first mask out any areas outside of California state boundaries using our `envlayer` object.

```{r mask sd, fig.height=5, fig.width=5, fig.align='center'}
# Perform masking
mgd <- mask_gd(kgd, envlayer)
...
|-------------------------|-----------------------------------------------------------|-------------------------------------------------|
| Manuscript with method  | [Bishop et al. 2022](REFER)                               | Paper describing wingen methodology and package |
