...
TODO: please see the wingen vignette and documentation for further details on this package.
...
# Map of data
plot(envlayers, col = magma(100), axes = FALSE, box = FALSE)

# Plot points on top
points(liz_coords, col = mako(1, begin = 0.4), pch = 3, cex = 0.5)
```

If users don't have a raster layer of their landscape, they can generate one from their coordinates using the `coords_to_raster()` function. The resolution of this raster can be either tuned with the `agg` (to aggregate) and `disagg` (to disaggregate) arguments or defined using the `res` argument.

```{r, fig.width = 5, fig.height = 5}
ex_raster1 <- coords_to_raster(liz_coords, buffer = 1, plot = TRUE)
ex_raster2 <- coords_to_raster(liz_coords, buffer = 1, agg = 2, plot = TRUE)
ex_raster3 <- coords_to_raster(liz_coords, buffer = 1, disagg = 4, plot = TRUE)
```
...
kgd <- krig_gd(wgd, index = 1:2, envlayers, disagg_grd = 2)
