...
Euclidean, Bray-Curtis, and PC-based genetic distance metrics ***require no missing data***. A simple imputation (on the median) is built into the `gen_dist_calc()` function mostly for creating test datasets; we ***highly discourage*** using this form of imputation in any of your analyses.

To calculate Euclidean distances between our samples:

```{r euc dists}
# Calculate Euclidean distance matrix
euc_dists <- gen_dist_calc(vcf_file = here("inst", "extdata", "liz_test.vcf"), dist_type = "euclidean")
...
```{r plink dists}
plink_dists <- gen_dist_calc(plink_file = here("inst", "extdata", "liz_test.dist"), plink_id_file = here("inst", "extdata", "liz_test.dist.id"), dist_type = "plink")
...
```{r pc dists}
pc_dists <- gen_dist_calc(vcf_file = here("inst", "extdata", "liz_test.vcf"), dist_type = "pc")
# TODO: switch above to system.file
```
