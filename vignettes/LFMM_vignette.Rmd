...
There are a variety of ways to determine the best number of latent factors in LFMM (which are called K values, although these are *not* the same as TESS K values). algatr provides the option to test using three such methods: (1) Tracy-Widom test; (2) quick elbow test (i.e., scree test); and (3) using the clustering algorithm from TESS (i.e., determining latent factors corresponding to some measure of population structure). We will explore each of these, and compare results, in this vignette.
...
One other argument to be aware of within `lfmm_run()` is the adjustment of p-values, specified using `p-adj`. There are two options for `p_adj`: `"none"`, which XXX (this is the default), and `"fdr"`, which adjusts p-values based on the false discovery rate. *TODO [EAC]: FILL IN ADJUSTED p-VALUES*
...
A QQplot plots theoretical quantiles against p-value quantiles, providing a way to visualize the significance of our outlier SNPs. The line that is plotted runs through the intercept and has a slope of 1:
...
```{r do everything, warning = FALSE,  results = FALSE, message = FALSE, warning = FALSE, fig.height=4, fig.width = 6, fig.align='center'}
