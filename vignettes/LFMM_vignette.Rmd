...
To run LFMM, we will need a genotype dosage matrix and environmental values extracted at sampling coordinates. Let's load the example dataset, convert the vcf to a dosage matrix using the `vcf_to_dosage()` function in the wingen package, and extract environmental variables based on our sampling coordinates.
...
select_K(gen, K_selection = "find_clusters", perc.pca = 90, max.n.clust = 10) # 7
```
