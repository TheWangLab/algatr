...
TODO [EAC]: Are 1. and 3. necessary to go through in a vignette (i.e., forward-facing)?

1.    Converting a vcf file to a dosage matrix (the `vcf_to_dosage()` function) and to a genind object (the `vcf_to_genind()` function)

2.    Pruning variants that are in linkage disequilibrium with one another (the `ld_prune()` function); this uses the snpRelate package:
...
TODO [EAC]: describe temp files created within ld_prune function and that they'll be removed
...
TODO [EAC]: deal with this section; consider removing
