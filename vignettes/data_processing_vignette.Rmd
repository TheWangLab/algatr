...
ld_prune(vcf = system.file("extdata", "liz_test.vcf", package = "algatr"), out_name = "liz", out_format = "vcf", slide.max.n=10)

# The above should have generated a new directory (liz_LDpruned) containing several files
# TODO[EAC]: it is highly unlikely that this will be created in a way that can be read in consistently. I would out the liz_LDpruned_r0.6_n10.vcf  in extdata as an example and read it in as a system file as follows. For example, I get an error when I ran the old code because it creates the LD folder in the algatr main folder but then tries to read it in from the vignette folder (where the Rmd is). In general, creating and reading in files from a users environment should be avoided, but I get that this is somewhat unavoidable in this case.
# Let's take a look at the LD-pruned vcf:
vcf_ldpruned <- vcfR::read.vcfR(system.file("extdata", "liz_LDpruned_r0.6_n10.vcf", package = "algatr"))
