...
**A** **L**andscape **G**enomic **A**nalysis **T**oolkit in **R** (**algatr**) was built to provide researchers with a step-by-step, start-to-finish pipeline to perform various landscape genomics methods with their data. The algatr package is described in [Chambers, Bishop, & Wang 2022](REFER).
...
Landscape genetics (or genomics) combines the fields of landscape ecology and population genetics in an effort to understand how environmental variation affects spatial genetic variation. Thus, at its most basic, any landscape genetics method requires genetic and environmental data as input.
...
algatr makes use of several existing packages and methods, and we provide citations to these packages (and corresponding publications) whenever possible. Many of these packages have extensive documentation and excellent additional resources, which we provide links to in the corresponding vignettes.

We have added functionality to each of these methods within algatr which we discuss in each of the methods' vignettes (and in our paper: [Chambers et al. 2022](REFER)).

Other than input data processing functions, the main functions within algatr are named `method_do_everything()`. As the name implies, these functions will take you from your raw input data through to generating results, tables, and figures from the analysis. For example, `gdm_do_everything()` will run generalized dissimilarity modeling, while also generating a GDM map, a table with results, and several other outputs. However, to better understand what's going on under the hood of these `method_do_everything()` functions, the algatr vignettes provide a line-by-line breakdown of the individual functions contained within the `method_do_everything()` function to (a) increase a user's understanding of how the method actually works, and (b) allow users with more customizability in how they run their own analysis, if so desired.

When deciding on methods to have within algatr, we found it best to first identify the questions that these methods seek to answer, and we think this is a good framework for anyone (particularly beginner landscape genomicists) to think about landscape genomics methods. These questions fall into four broad categories of analyses.

+-------------------------------------------------------------------------------+----------------------------------------------------------+----------------------------------------------------------------------------------------------------------------+---------------------+--------------------------+
| Question                                                                      | Category                                                 | Method                                                                                                         | Vignette            | algatr function          |
+===============================================================================+==========================================================+================================================================================================================+=====================+==========================+
| How is genetic variation structured spatially?                                | Structure                                                | TESS (Caye et al. 2016)                                                                                        | TESS_vignette.Rmd   | `tess_do_everything()`   |
+-------------------------------------------------------------------------------+----------------------------------------------------------+----------------------------------------------------------------------------------------------------------------+---------------------+--------------------------+
| What are the effects of geography and environment on genetic differentiation? | Isolation by distance/isolation by environment (IBD/IBE) | Multiple matrix regression with randomization; MMRR (Wang 2013)                                                | MMRR_vignette.Rmd   | `mmrr_do_everything()`   |
|                                                                               |                                                          |                                                                                                                |                     |                          |
...
+-------------------------------------------------------------------------------+----------------------------------------------------------+----------------------------------------------------------------------------------------------------------------+---------------------+--------------------------+
| What regions of the genome show evidence of climate associations?             | Genotype-environment associations (GEA)                  | Redundancy analysis; RDA (Capblancq & Forester 2021)                                                           | RDA_vignette.Rmd    | `rda_do_everything()`    |
|                                                                               |                                                          |                                                                                                                |                     |                          |
|                                                                               |                                                          | Latent factor mixed models; LFMM (Caye et al. 2019)                                                            | LFMM_vignette.Rmd   | `lfmm_do_everything()`   |
+-------------------------------------------------------------------------------+----------------------------------------------------------+----------------------------------------------------------------------------------------------------------------+---------------------+--------------------------+
| How is intraspecific genetic diversity distributed spatially?                 | Genetic diversity                                        | Moving windows of genetic diversity; wingen (Bishop et al. 2022)                                               | wingen_vignette.Rmd | `wingen_do_everything()` |
+-------------------------------------------------------------------------------+----------------------------------------------------------+----------------------------------------------------------------------------------------------------------------+---------------------+--------------------------+
...
terra::plot(CA_env, col = turbo(100), axes = FALSE)
```
...
CA_env_sp <- terra::rast(CA_env)

for(layer in 1:3){
  minval <- terra::minmax(CA_env_sp[[layer]])[1,]
  maxval <- terra::minmax(CA_env_sp[[layer]])[2,]
  
  CA_env_sp[[layer]] <- ((CA_env_sp[[layer]] - minval) / (maxval - minval))*255
}

terra::plotRGB(CA_env_sp, r = 1, g = 2, b = 3)
...
You'll now want to do some processing of these data, such as file conversions and LD-pruning (see the [**data processing vignette**](REFER)) and calculating genetic distances (see the [**genetic distances vignette**](REFER)).

+----------+----------------------------------------------------------------+--------------------------+-------------------------------------------+---------------+
|          | R package                                                      | algatr function          | Input files                               | Required args |
+==========+================================================================+==========================+===========================================+===============+
| TESS     | [tess3r](https://bcm-uga.github.io/TESS3_encho_sen/index.html) | `tess_do_everything()`   | -   Genotype dosage matrix                |               |
|          |                                                                |                          | -   Environmental layers                  |               |
|          |                                                                |                          | -   Sampling coordinates                  |               |
+----------+----------------------------------------------------------------+--------------------------+-------------------------------------------+---------------+
| MMRR     | algatr                                                         | `mmrr_do_everything()`   | -   Genetic distance matrix               |               |
|          |                                                                |                          |                                           |               |
|          |                                                                |                          | -   Environmental layers                  |               |
|          |                                                                |                          |                                           |               |
|          |                                                                |                          | -   Sampling coordinates                  |               |
+----------+----------------------------------------------------------------+--------------------------+-------------------------------------------+---------------+
| GDM      | [gdm](https://cran.r-project.org/web/packages/gdm/index.html)  | `gdm_do_everything()`    | -   Genetic distance matrix               |               |
|          |                                                                |                          | -   Environmental layers                  |               |
|          |                                                                |                          | -   Sampling coordinates                  |               |
+----------+----------------------------------------------------------------+--------------------------+-------------------------------------------+---------------+
| RDA      | TODO                                                           | `rda_do_everything()`    | -   Genotype dosage matrix                |               |
|          |                                                                |                          |                                           |               |
|          |                                                                |                          | -   Environmental layers                  |               |
|          |                                                                |                          |                                           |               |
|          |                                                                |                          | -   Sampling coordinates                  |               |
+----------+----------------------------------------------------------------+--------------------------+-------------------------------------------+---------------+
| LFMM     | [LEA](https://rdrr.io/bioc/LEA/)                               | `lfmm_do_everything()`   | -   Genotype dosage matrix                |               |
|          |                                                                |                          | -   Environmental layers                  |               |
|          |                                                                |                          | -   Sampling coordinates                  |               |
+----------+----------------------------------------------------------------+--------------------------+-------------------------------------------+---------------+
| wingen   | wingen                                                         | `wingen_do_everything()` | -   VCF file                              |               |
