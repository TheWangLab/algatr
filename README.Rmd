...
We can combine all three PCs into a single map by scaling each of the rasters in such a way that they each correspond to either R, G, or B using the `scaleRGB()` function, and subsequently map using the `plotRGB()` function.

```{r, RGB plot, fig.width = 5, fig.height = 6, fig.align='center'}
env <- scaleRGB(CA_env)
...
|          |                                                                   |                          | -   Sampling coordinates                  |               |
+----------+-------------------------------------------------------------------+--------------------------+-------------------------------------------+---------------+
| RDA      | [vegan](https://cran.r-project.org/web/packages/vegan/index.html) | `rda_do_everything()`    | -   Genotype dosage matrix                |               |
|          |                                                                   |                          |                                           |               |
|          |                                                                   |                          | -   Environmental layers                  |               |
|          |                                                                   |                          |                                           |               |
|          |                                                                   |                          | -   Sampling coordinates                  |               |
