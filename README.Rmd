...
algatr depends on many packages for all of the different methods implemented. In order to ensure that algatr can still be installed even if one of the many dependencies breaks, the packages required for each method must be installed separately from algatr. This is also helpful if you are only interested in using a subset of the methods provided and don't want to install a bunch of unnecessary extra packages. Below we list the packages that you need to install for each part of algatr.
...
Alternatively, algatr can be run using [Docker](https://docs.docker.com/get-started/). [Docker](https://docs.docker.com/get-started/) must first be installed and then you can start algatr within a Docker container: 
```bash
...
```
Open localhost:8787 in your browser and log in with user:algatr and password:yourpassword (substitute yourpassword for whatever password you would like)
