...
The algatr package depends on many packages for all of the different methods implemented. To ensure that algatr can still be installed even if one of the many dependencies breaks, the packages required for each method must be installed separately from algatr. This is also helpful if you're only interested in using a subset of the methods provided and don't want to install unnecessary packages. Below, we list the packages that you need to install for each part of algatr.
...
Once you have installed the above dependencies, you can install algatr using the following code:
